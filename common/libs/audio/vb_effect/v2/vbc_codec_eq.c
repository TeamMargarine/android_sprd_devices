...
LOCAL void init_regs()
{
	...
	ADHPCTL =  (VB_AD_CTL_MAP_BASE + 0x0004);
	 ...
	 DADGCTL =(uint32_t) &reserved_resigter[0];
	 ADPATCHCTL =(uint32_t) &reserved_resigter[1];
	 ADDG01CTL = (uint32_t)&reserved_resigter[2];
	 ADDG23CTL =(uint32_t) &reserved_resigter[3];
	 ADHPCTL =(uint32_t) &reserved_resigter[4];
	 ...
	 )
 {
	...
	VB_AD_CTL_MAP_BASE = (uint32_t)ad_ctl_buf_ptr;
	init_regs();
