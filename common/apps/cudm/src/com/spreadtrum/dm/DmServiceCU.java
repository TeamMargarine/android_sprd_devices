...
public class DmServiceCU extends Service {
    ...
    private static int pppstatus = Phone.APN_TYPE_NOT_AVAILABLE;
...
//    }
    ...
    private void createDmApn() {
		...
		(curPhoneId == 0)?Telephony.Carriers.CONTENT_URI:Telephony.Carriers.CONTENT_URI_SIM2, null,
                selection, null, null);
    ...
    public void setAPN(Context context, String apn) {
     ...
     //   if (!Vdmc.getInstance().isVDMRunning()) {
            ...
            int c = values.size() > 0 ? context.getContentResolver().update(
                    (curPhoneId == 0)?Telephony.Carriers.CONTENT_URI:Telephony.Carriers.CONTENT_URI_SIM2, values, selection, null) : 0;
