...
public class DmReceiverCU extends BroadcastReceiver {

    protected static final String DM_TAG = "DM ==> ";

    private String TAG = DM_TAG + "DmReceivercu: ";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        } else if (action.equals("com.android.dm.stop"))
        {
            Log.d(TAG, "onReceive, com.android.dm.stop");        
          Intent selfRegService = new Intent("com.android.dm.SelfReg");
          context.stopService(selfRegService);
        }
/*
...
*/
    ...
    private boolean isDmSmsAddress(Context context, String addr) {
        ...
        String selfRegSmsAddr = DmServiceCU.getInstance().getSmsAddr();
    ...
    private boolean parseReplyMsgContent(Context context, String msgBody, int phoneId) {
        ...
        String imei = DmServiceCU.getInstance().getImei(phoneId);
        String okReply = "IMEI:" + imei + "/1";
        String failReply = "IMEI:" + imei + "/0";
        DmServiceCU dmService = DmServiceCU.getInstance();
	Log.d(TAG, "parseReplyMsgContent phoneid:"+phoneId +"IMSI:"+imsi);
	...
	if (!DmServiceCU.getInstance().isNeedSelfReg())
		{
	            Log.d(TAG, "processDataSms: register all OK");
	          Intent selfRegService = new Intent("com.android.dm.SelfReg");
	          context.stopService(selfRegService);		    
		}
        return result;
    ...
    private boolean processWapPush(Context context, Intent intent) {
        ...
        String dmSmsAddr = DmServiceCU.getInstance().getSmsAddr();
