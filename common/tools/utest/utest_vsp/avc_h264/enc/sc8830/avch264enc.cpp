...
static int enc_encode_frame(unsigned char* py, unsigned char* py_phy, unsigned char* puv, unsigned char* puv_phy, unsigned char* pframe, unsigned int* size, unsigned char* *pyuv_rec, unsigned int timestamp, int* type, int bs_remain_len = 0)
{
	MMEncIn vid_in;
	MMEncOut vid_out;
	int ret;
...
//	}
	...
	while (!feof(fp_yuv))
	{
		...
		int ret = enc_encode_frame(pyuv, pyuv_phy, pyuv+width*height, pyuv_phy+width*height, frame_buffer, &frame_size, &pyuv_rec, framenum_bs*1000/framerate, &type, bs_remain_len);
		if (ret != 0)
		{
